...
export const seedQuestions = async (dataSource: DataSource) => {
  ...
  const optionRepository = dataSource.getRepository(Option); // <-- Get the Option repository

  const questionsData = [
    {
      // id: 1, // Remove explicit ID, let TypeORM handle it
  ...
  if (existingQuestions === 0) {
    ...
    for (const qData of questionsData) {
      // 1. Create and save the Question entity
      const question = questionRepository.create({ text: qData.text });
      await questionRepository.save(question); // Save to get its generated ID

      // 2. Create and save the associated Option entities
      const optionsToSave = qData.options.map((oData) => {
        return optionRepository.create({
          text: oData.text,
          correspondence: oData.correspondence,
          question: question, // Link to the saved Question entity
        });
      });
